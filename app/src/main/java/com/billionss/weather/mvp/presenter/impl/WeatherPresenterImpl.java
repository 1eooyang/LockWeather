...
public class WeatherPresenterImpl extends BasePresenter<RxWeatherView> {
     ...
     * 将获取的数据传递给View进行界面赋值
     */
    public void getWeatherListData(final String area, String needMoreDay, String needIndex,
        ...
        weatherInteractor.getWeatherData(area, needMoreDay, needIndex, needAlarm, need3HourForcast, new OnRxWeatherLisenter() {
            ...
            public void onSuccess(ShowApiWeather data) {
             /*   if (data == null) {
                ...
                ACache.get(BaseApplication.getApplication()).put(area,data);
                //成功获取后进行数据初始化
            }

            @Override
            public void onError() {
                ShowApiWeather asObject = ACache.get(BaseApplication.getApplication()).getAsObject(area);
                if (asObject != null) {
                    onSuccess(asObject);
                } else {
